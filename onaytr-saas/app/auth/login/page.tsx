    ...
    try {
      ...
      if (res?.error) {
        ...
        const errorMap: Record<string, string> = {
          'CredentialsSignin': t('auth_err_credentials'),
          'Configuration': t('auth_err_server'),
          'AccessDenied': t('auth_err_denied'),
          'Verification': t('auth_err_verify'),
          'Default': t('auth_err_default'),
        };
        const msg = errorMap[res.error] || res.error || t('auth_err_generic');
        setError(msg);
        setIsLoading(false);
      } else {
        router.push('/');
    ...
    <div className="min-h-screen flex items-center justify-center relative overflow-hidden px-4 bg-[#F8F9FF] dark:bg-[#0D1117] pt-20">
      ...
      <div className="w-full max-w-md glass-premium p-8 md:p-10 rounded-3xl shadow-xl relative group overflow-hidden">
        ...
        <div className="flex flex-col items-center mb-8 relative z-10">
          ...
          <h1 className="text-3xl font-black text-slate-800 dark:text-white display-font mb-2">{t('auth_welcome')}</h1>
          <p className="text-slate-500 dark:text-slate-455 text-sm font-medium text-center">{t('auth_login_desc')}</p>
        </div>
        ...
        <form onSubmit={handleSubmit} className="space-y-5 relative z-10">
          ...
          {show2FA && (
            <div className="space-y-1.5 animate-fade-in text-left">
              <label className="text-[13px] font-black text-[#4648d4] dark:text-[#712ae2] ml-1">{t('auth_2fa_label')}</label>
              <div className="relative group/input">
                <div className="absolute inset-y-0 left-0 pl-4 flex items-center pointer-events-none">
                ...
                <input
                  ...
                  required
                  className="w-full pl-11 pr-4 py-3.5 bg-[#4648d4]/5 border border-[#4648d4]/20 focus:border-[#4648d4] focus:ring-2 focus:ring-[#4648d4]/10 rounded-xl text-slate-800 dark:text-white placeholder-slate-400 font-mono tracking-widest text-center text-lg font-black outline-none transition-all shadow-sm"
                  placeholder="000000"
                />
              </div>
              <p className="text-[10px] text-slate-500 font-bold ml-1">{t('auth_2fa_hint')}</p>
            </div>
          )}

          <div className="flex items-center justify-between pt-2">
            <label className="flex items-center gap-2 cursor-pointer">
              <input type="checkbox" className="w-4 h-4 rounded border-slate-350 dark:border-slate-800 text-[#4648d4] focus:ring-[#4648d4]/30 bg-white/50 dark:bg-slate-900" />
              <span className="text-xs font-bold text-slate-650 dark:text-slate-400">{t('auth_remember')}</span>
            </label>
            <Link href="/auth/forgot-password" className="text-xs font-bold text-[#4648d4] hover:text-[#712ae2] hover:underline">
              {t('auth_forgot')}
            </Link>
          </div>
          ...
          >
            {isLoading ? (
              ...
              <>
                {t('auth_login_btn')} <ArrowRight className="h-5 w-5" />
              </>
            )}
          </button>
        </form>
        ...
        <div className="mt-8 text-center relative z-10">
          <p className="text-sm font-medium text-slate-500 dark:text-slate-450">
            {t('auth_no_account')}{' '}
            <Link href="/auth/register" className="font-bold text-[#4648d4] hover:text-[#712ae2] hover:underline">
