...
export default function BulkCountryActions() {
  ...
  const handleBulkUpdate = async (isActive: boolean) => {
    const confirmMsg = `Tüm ülkeleri toplu olarak ${isActive ? 'AKTİF' : 'PASİF'} yapmak istediğinize emin misiniz?`;
  ...
  const handleBulkMargin = async (e: React.FormEvent, provider: '5sim' | 'herosms') => {
    ...
    const confirmMsg = `Tüm ülkelerin ${provider === '5sim' ? '5Sim' : 'HeroSMS'} özel kâr oranını %${val || 'global (temizle)'} yapmak istediğinize emin misiniz?`;
    if (!confirm(confirmMsg)) return;
    ...
    <div className="flex flex-wrap items-center gap-2">
      ...
      {showHerosmsInput ? (
        ...
        <button
          ...
          disabled={isLoading}
          className="flex items-center gap-1.5 px-3 py-2 bg-indigo-550 hover:bg-indigo-600 disabled:opacity-50 text-indigo-700 text-sm font-bold rounded-xl transition-colors shadow-sm"
        >
          <Percent className="h-4 w-4" />
